...
def get_info_from_current_page(page:int):
    ...
    for advert in list(bs.find_all('div', {'class':'listing__teaserWrapper'})):
        ...
        #         .get_text(strip=True)\
        #         .lower()
        #     advert_dict['pokoje'] = rooms_text.split(' ')[0]

        # floor = rooms.find_next_siblings('li', {'class':'teaserUnified__listItem'})
        # if floor:
        #     floor_text = floor\
        #         .get_text(strip=True)\
        #         .lower()
        #     advert_dict['pietro'] = floor_text.split(' ')[0].replace('parter', '0')
